...
const House_owner_main_page = () => {
  ...
  const [home_data, setHomedata] = useState(true);
  ...
  const Amount_send_fun = async (list) => {
    try {
      ...
      await updateDoc(
        ...
        {
          Room_booker_id: "true",
          amount_send: `${new Date().getFullYear()}-${
            new Date().getMonth() + 1
          }-${
            new Date().getDate() <= 9
              ? `0${new Date().getDate()}`
        ...
        swal.fire(
          "SUCCESS!!!",
          `Total amount is ${
            list.Total_rend - (list.Total_rend * 10) / 100
          } send successfull`,
          "success"
        )
      );
                    ...
                    <Card>
                      ...
                      <Card.Content extra>
                        <div className="amount_resend">
                          {list.Room_booker_id === "true" ? (
                            <Button
                              primary
                              id="mailltouser"
                              onClick={() => Amount_send_fun(List)}
                            >
                              <Icon name="linkedin" /> amount send
                            </Button>
                          ) : (
                            <Button
                              primary
